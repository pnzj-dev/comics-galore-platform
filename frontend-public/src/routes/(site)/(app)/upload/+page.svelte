<script lang="ts">
	...
	import CompactCard from '$lib/components/comics/CompactCard.svelte';
	import ComicForm from '$lib/components/upload/ComicForm.svelte';
	import ArchiveForm from '$lib/components/upload/ArchiveForm.svelte';
	...
	const uploadMode = $derived(data.uploadMode as 'direct' | 'backend');
	const pagePreviewThreshold = $derived(data.pagePreviewThreshold as number);
	const uploadPartSizeMB = $derived(data.uploadPartSizeMB as number);
	const uploadConcurrency = $derived(data.uploadConcurrency as number);
...
<section class="py-8">
	...
	{:else if tab === 'manual'}
		...
		<ComicForm uploadMode={uploadMode} pagePreviewThreshold={pagePreviewThreshold} uploadPartSizeMB={uploadPartSizeMB} uploadConcurrency={uploadConcurrency} />
	{:else if tab === 'archive'}
		<h2 class="text-2xl font-semibold mb-4">Upload Archive</h2>
		<ArchiveForm uploadMode={uploadMode} uploadPartSizeMB={uploadPartSizeMB} uploadConcurrency={uploadConcurrency} />
	{/if}
</section>
