<script lang="ts">
	...
	async function createTicket() {
		...
		error = '';
		try {
			const t = await encore.social.CreateTicket({ subject, body, priority: 'normal', turnstile_token: turnstileToken || '' });
			subject = '';
			body = '';
	...
	async function sendReply() {
		...
		try {
			const m = await encore.social.ReplyTicket(activeId, { body: reply, turnstile_token: turnstileToken || '' });
			thread = [...thread, m];
			reply = '';
		} catch (e) {
		...
		<div class="space-y-4">
			<Card>
				...
				<CardContent class="space-y-3">
					...
					<Turnstile action="support_ticket" onToken={(t) => (turnstileToken = t)} resetSignal={turnstileReset} />
					<Button class="w-full" onclick={createTicket} disabled={creating || (turnstileRequired && !turnstileToken)}>{creating ? 'Submitting…' : 'Submit ticket'}</Button>
				</CardContent>
			</Card>
		...
		<div class="rounded-lg border border-border flex flex-col h-[60vh]">
			{#if activeId}
				...
				<div class="border-t border-border p-3 flex gap-2">
					...
					<Button onclick={sendReply} disabled={!reply.trim() || (turnstileRequired && !turnstileToken)}>Reply</Button>
				</div>
			{:else}
