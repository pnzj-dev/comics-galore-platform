...
// sanitizeFilename builds a URL/filesystem-safe base filename from the comic's
// display parts: "author - title - volume - issue number", skipping empty
// parts. "#12" is mapped to "no-12", and URL/filesystem-unsafe characters are
// replaced with underscores.
export function sanitizeFilename(parts: (string | undefined | null)[]): string {
...
// buildDownloadFilename builds a .cbz filename from the comic's display parts.
export function buildDownloadFilename(parts: (string | undefined | null)[]): string {
	return `${sanitizeFilename(parts)}.cbz`;
}
