<script lang="ts">
	...
	let { comments, onReply, onDelete, userId, role = 'user', depth = 0 }: Props = $props();
...
{#if comments.length > 0}
	<div class="space-y-3" class:ml-6={depth > 0} class:border-l-2={depth > 0} class:border-border={depth > 0} class:pl-4={depth > 0}>
			...
			<div class="group">
				<div class="flex items-start gap-2">
					...
					<div class="flex-1 min-w-0">
						...
						<button onclick={() => onReply(comment.id)} class="text-[10px] text-muted-foreground hover:text-primary mt-1">Reply</button>
					</div>
				</div>
